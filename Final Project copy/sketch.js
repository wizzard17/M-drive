...
let listofblocks=[1,2,3,4,5,6,7,8                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        ]
let player1
let player2
let futureblocks=[]
let blockstypelist=[]
...
class DrawGrid {
  ...
  falling() {
    ...
    for (let y = numRows - 1; y >= 0; y--) {
      for (let x = numCols - 1; x >= 0; x--) {
        if (y < numRows - 1 && this.gridData[y][x] !== 0 && this.gridData[y + 1][x] === 0) {
  ...
  // Function to break matching gems (or orbs) on the board
  breakMatchingOrbs(row, col) {
    const color = this.gridData[row][col];
    if (color === 0) return; // No orb at this position
    
    const visited = new Array(this.gridData.length).fill(null).map(() => new Array(this.gridData[0].length).fill(false));
    this.break(this.gridData, row, col, color, visited);
  }

  // Recursive function to break orbs based on adjacency and color matching
  break(board, row, col, color, visited) {
    // Check boundaries and if the orb has already been visited
    if (row < 0 || row >= board.length || col < 0 || col >= board[0].length || visited[row][col] || board[row][col] !== color) {
      return;
    }

    // Mark the current orb as visited
    visited[row][col] = true;

    // Recursively check adjacent orbs (up, down, left, right)
    this.break(board, row - 1, col, color, visited); // Up
    this.break(board, row + 1, col, color, visited); // Down
    this.break(board, row, col - 1, color, visited); // Left
    this.break(board, row, col + 1, color, visited); // Right

    // After checking all adjacent orbs, "break" this orb (set to 0 to represent broken orb)
    board[row][col] = 0;
  }
}
class Drop{
